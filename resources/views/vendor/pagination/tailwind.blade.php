@if ($direction->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        <div class="flex flex-1 justify-between sm:hidden">
            @if ($direction->onFirstPage())
                <span
            ...
            @else
                <a href="{{ $direction->previousPageUrl() }}"
            ...
            @if ($direction->hasMorePages())
                <a href="{{ $direction->nextPageUrl() }}"
        ...
        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
                <p class="text-sm leading-5 text-gray-700">
                    {!! __('Showing') !!}
                    @if ($direction->firstItem())
                        <span class="font-medium">{{ $direction->firstItem() }}</span>
                        {!! __('to') !!}
                        <span class="font-medium">{{ $direction->lastItem() }}</span>
                    @else
                        {{ $direction->count() }}
                    @endif
                    {!! __('of') !!}
                    <span class="font-medium">{{ $direction->total() }}</span>
                    {!! __('results') !!}
                </p>
            </div>
            ...
            <div>
                <span class="relative z-0 inline-flex rounded-md shadow-sm">
                    ...
                    @else
                        <a href="{{ $direction->previousPageUrl() }}" rel="prev"
                    ...
                    {{-- Next Page Link --}}
                    @if ($direction->hasMorePages())
                        <a href="{{ $direction->nextPageUrl() }}" rel="next"
